...
class trip_schedule():
    ...
    def getAirport(self, city_code):
        return self.db_access.getAirportByCity(city_code)
    ...
    def doFlightSearch(self, origin, dest, flight_date):
        ...
        api = flight_api(trip_class=trip_class, adults=adults, children=children, infants=infants, origin=origin, destination=destination, start_date=start_date)

        flight_result = api.flight_search()
        ticket_data = flight_api.sorted_one_way_flight_result(flight_result)
        self.flight_plan.append(json.dumps(ticket_data))
